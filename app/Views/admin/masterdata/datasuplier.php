...
<body>
			...
			<div class="min-height-200px">
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">Tabel Data Suplier</h4>
                            <a href="<?= base_url('/insert_suplier') ?>" class="btn btn-primary" role="button">Insert Data +</a>
						</div>	
					</div>
					<table class="table table-bordered">
						...
						<tbody>
							<?php 
							$no = 1;
							foreach($all_data_suplier as $suplier) : ?>
							<tr>
								<td><?= $no++; ?></td>
								<td><?= $suplier->nama ?></td>
								<td><?= $suplier->alamat ?></td>
								<td><?= $suplier->no_telp ?></td>
								<td><a href="<?= base_url('/edit_suplier'). '/' .$suplier->id_suplier ?>" class="btn btn-warning" role="button">Edit</a>
								<a href="#" onclick="showDeleteConfirmation(<?= $suplier->id_suplier ?>)" class="btn btn-danger" role="button">Delete</a>
							</tr>
							<?php endforeach ?>
						</tbody>
					</table>
	...
	<script>
		function showDeleteConfirmation(id_suplier) {
			Swal.fire({
				title: 'Apakah Anda yakin?',
				text: 'Data suplier obat ini akan dihapus permanen!',
				icon: 'warning',
		...
		function deleteSuplier(id_suplier) {
			$.ajax({
				url: `<?= base_url('/delete_suplier/') ?>${id_suplier}`,
				method: 'GET',
				success: function(response) {
					Swal.fire(
						'Terhapus!',
						'Data suplier obat telah dihapus.',
						'success'
					).then(() => {
						// Redirect atau lakukan sesuatu setelah data dihapus
						window.location.href = `<?= base_url('/data_suplier') ?>`;
					});
				},
				error: function() {
					Swal.fire(
						'Gagal!',
						'Terjadi kesalahan saat menghapus data suplier obat.',
						'error'
					);
				}
			});

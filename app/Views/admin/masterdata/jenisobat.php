...
<body>
			...
			<div class="min-height-200px">
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">Tabel Data Jenis Obat</h4>
                            <a href="<?= base_url('/insert_jenis') ?>" class="btn btn-primary" role="button">Insert Data +</a>
						</div>	
					</div>
					<table class="table table-bordered">
						...
						<tbody>
							<?php 
							$no =1;
							foreach ($all_data_jenis as $jenis) : ?>
								<tr>
									<td><?= $no++; ?></td>
									<td><?= $jenis->jenis ?></td>
									<td><a href="<?= base_url('/edit_jenis'). '/' .$jenis->id_jenis ?>" class="btn btn-warning" role="button">Edit</a>
									<a href="#" onclick="showDeleteConfirmation(<?= $jenis->id_jenis ?>)" class="btn btn-danger" role="button">Delete</a>
								</tr>
							<?php endforeach ?>
						</tbody>
					</table>
	...
	<script>
		function showDeleteConfirmation(id_jenis) {
			Swal.fire({
				title: 'Apakah Anda yakin?',
				text: 'Data jenis obat ini akan dihapus permanen!',
				icon: 'warning',
				...
				if (result.isConfirmed) {
					...
					deleteJenis(id_jenis);
				}
			});
		}
		function deleteJenis(id_jenis) {
			$.ajax({
				url: `<?= base_url('/delete_jenis/') ?>${id_jenis}`,
				method: 'GET',
				success: function(response) {
					Swal.fire(
						'Terhapus!',
						'Data jenis obat telah dihapus.',
						'success'
					).then(() => {
						// Redirect atau lakukan sesuatu setelah data dihapus
						window.location.href = `<?= base_url('/jenis_obat') ?>`;
					});
				},
				error: function() {
					Swal.fire(
						'Gagal!',
						'Terjadi kesalahan saat menghapus data jenis obat.',
						'error'
					);
				}
			});

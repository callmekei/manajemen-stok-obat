...
<body>
			...
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4>Data Satuan Obat</h4>
							</div>
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.html">Home</a></li>
									<li class="breadcrumb-item active" aria-current="page">Master Data</li>
									<li class="breadcrumb-item active" aria-current="page">Data Satuan Obat</li>
								</ol>
							</nav>
						</div>
					</div>
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">Tabel Data Satuan Obat</h4>
                            <a href="<?= base_url('/insert_satuan') ?>" class="btn btn-primary" role="button">Insert Data +</a>
						</div>	
					</div>
					<table class="table table-bordered">
						...
						<tbody>
							<?php 
							$no =1;
							foreach ($all_data_satuan as $satuan) : ?>
								<tr>
									<td><?= $no++; ?></td>
									<td><?= $satuan->satuan ?></td>
									<td><a href="<?= base_url('/edit_satuan'). '/' .$satuan->id_satuan ?>" class="btn btn-warning" role="button">Edit</a>
									<a href="#" onclick="showDeleteConfirmation(<?= $satuan->id_satuan ?>)" class="btn btn-danger" role="button">Delete</a>
								</tr>
							<?php endforeach ?>
						</tbody>
					</table>
	...
	<script>
		function showDeleteConfirmation(id_satuan) {
			Swal.fire({
				title: 'Apakah Anda yakin?',
				text: 'Data satuan obat ini akan dihapus permanen!',
				icon: 'warning',
		...
		function deleteSatuan(id_satuan) {
			$.ajax({
				url: `<?= base_url('/delete_satuan/') ?>${id_satuan}`,
				method: 'GET',
				success: function(response) {
					Swal.fire(
						'Terhapus!',
						'Data satuan obat telah dihapus.',
						'success'
					).then(() => {
						// Redirect atau lakukan sesuatu setelah data dihapus
						window.location.href = `<?= base_url('/satuan_obat') ?>`;
					});
				},
				error: function() {
					Swal.fire(
						'Gagal!',
						'Terjadi kesalahan saat menghapus data satuan obat.',
						'error'
					);
				}
			});
